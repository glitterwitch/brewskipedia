...
ContentTree = function(filename) {
  var groups, parent, plugin, _i, _len;
  parent = null;
  groups = {
    directories: []
  };
...
ContentTree.fromDirectory = function() {
  var arg, args, base, callback, directory, minimatchOptions, options, tree, _i, _j, _len;
  directory = arguments[0], args = 3 <= arguments.length ? __slice.call(arguments, 1, _i = arguments.length - 1) : (_i = 1, []), callback = arguments[_i++];
    ...
    async.apply(fs.readdir, directory), function(filenames, callback) {
      if (options.ignore != null) {
        return async.filter(filenames, function(filename, callback) {
          var include, pattern, relname, _k, _len1, _ref;
      ...
      return async.forEach(filenames, function(filename, callback) {
        ...
        return async.waterfall([
          async.apply(fs.lstat, filename), function(stats, callback) {
            var basename, i, match, plugin, relname, _k, _ref;
            if (stats.isDirectory()) {
              return ContentTree.fromDirectory(filename, base, options, function(error, result) {
                result.parent = tree;
              ...
              relname = path.relative(base, filename);
              match = false;
              for (i = _k = _ref = contentPlugins.length - 1; _k >= 0; i = _k += -1) {
                plugin = contentPlugins[i];
                if (minimatch(relname, plugin.pattern, minimatchOptions)) {
