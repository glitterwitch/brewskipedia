...
var async, defaults, fileExists, fs, logger, path, readJSON, _ref;
...
exports.fileExists = fileExists = fs.exists || path.exists;
  ...
  return async.waterfall([
    function(callback) {
      ...
      return async.forEach(options.require, function(moduleName, callback) {
        var moduleAlias;
        moduleAlias = moduleName.split('/').slice(-1);
      ...
      return async.forEach(paths, function(filepath, callback) {
        return fileExists(options[filepath], function(exists) {
          if (exists) {
            return callback();
          } else {
