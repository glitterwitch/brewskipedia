...
var ContentTree, async, colorCode, colors, extend, fs, loadPlugins, loadTemplates, logger, mime, path, run, setup, stripExtension, url, util, _ref, _ref1;
...
_ref1 = require('./'), loadTemplates = _ref1.loadTemplates, loadPlugins = _ref1.loadPlugins, ContentTree = _ref1.ContentTree;
...
setup = function(options, callback) {
  ...
  */
  ...
  var contentHandler, contentOptions, requestHandler;
  contentOptions = {
    ...
    return async.waterfall([
      function(callback) {
        ...
        return async.detect(ContentTree.flatten(contents), function(item, callback) {
          return callback(uri === item.url || (item.url[item.url.length - 1] === '/' && uri === (item.url + 'index.html')));
        }, function(result) {
          if (result) {
...
run = function(options) {
  ...
  return async.waterfall([async.apply(loadPlugins, options.plugins)], function(error) {
